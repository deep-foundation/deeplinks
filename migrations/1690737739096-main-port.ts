...
import { packageExists } from './1664940577200-tsx.js';
...
export const up = async () => {
  log('up');
  const packageName = '@deep-foundation/main-port';
  if (!await packageExists(packageName)) {
    ...
    const packageId = await installPackage(deep, '@deep-foundation/main-port');
  }
};
