...
const debug = Debug('deeplinks:migrations:payments');
...
export const up = async () => {
  log('up');
  const packageName = '@deep-foundation/payments';
  if (!await packageExists(packageName)) {
    ...
    await installPackage(deep, '@deep-foundation/payments');
    const packageId = await root.id('@deep-foundation/payments');
