...
export const up = async () => {
  log('up');
  const packageName = '@deep-foundation/authorization';
  if (!await packageExists(packageName)) {
    ...
    await installPackage(deep, '@deep-foundation/authorization');
    const packageId = await root.id('@deep-foundation/authorization');
