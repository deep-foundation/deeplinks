...
const debug = Debug('deeplinks:migrations:unsafe');
...
export const up = async () => {
  log('up');
  const packageName = '@deep-foundation/unsafe';
  if (!await packageExists(packageName)) {
    ...
    await installPackage(deep, '@deep-foundation/unsafe');
    const packageId = await root.id('@deep-foundation/unsafe');
