...
const debug = Debug('deeplinks:migrations:passport-username-password');
...
export const up = async () => {
  log('up');
  const packageName = '@deep-foundation/passport-username-password';
  if (!await packageExists(packageName)) {
    ...
    await installPackage(deep, '@deep-foundation/passport-username-password');
    const packageId = await root.id('@deep-foundation/passport-username-password');
