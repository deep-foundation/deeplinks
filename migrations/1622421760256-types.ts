...
export const up = async () => {
  ...
  if (errors?.length) {
    ...
    throw new Error(`Import error: ${JSON.stringify(errors[0]?.graphQLErrors?.[0]?.message || errors?.[0])}${error?.message ? ` ${error?.message} ${error?.request?.method} ${error?.request?.host}:${error?.request?.port}${error?.request?.path}` : ''}`);
  } else {
    await root.insert({
