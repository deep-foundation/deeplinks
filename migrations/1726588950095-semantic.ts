...
const debug = Debug('deeplinks:migrations:semantic');
...
export const up = async () => {
  log('up');
  const packageName = '@deep-foundation/semantic';
  if (!await packageExists(packageName)) {
    ...
    await installPackage(deep, '@deep-foundation/semantic');
    const packageId = await root.id('@deep-foundation/semantic');
