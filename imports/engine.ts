...
export async function call (options: IOptions) {
  ...
  if (options.operation === 'run') {
    const { stdout, stderr } = await execP(`((cd ${_hasura}/local/; npm run docker); sleep 10; (cd ${_deeplinks}; npm run migrate))`);
    console.log(stdout);
    console.log(stderr);
  }
  if (options.operation === 'sleep') {
    const { stdout, stderr } = await execP(`(cd ${_hasura}/local/; docker-compose down;)`);
    console.log(stdout);
    console.log(stderr);
  }
  if (options.operation === 'reset') {
    const { stdout, stderr } = await execP(`((cd ${_hasura}/local/; docker rm -f $(docker ps -a -q); docker volume rm $(docker volume ls -q)); (cd ${_deeplinks}; find . -type f -name '.migrate' -exec rm {} +))`);
    console.log(stdout);
    console.log(stderr);
  }
