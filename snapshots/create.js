...
const create = async () => {
  ...
  const { stdout, stderr } = await execP(`(docker compose -f docker-compose.yml -p deep stop || true) && docker run -v ${_deeplinks}:/deeplinks --volumes-from deep-postgres --rm --name links --entrypoint \"sh\" deepf/deeplinks:main -c \"cd / && ls var/lib/postgresql && tar -c -v -C /var/lib/postgresql -f /deeplinks/snapshots/${time} ./data" && docker compose -f docker-compose.yml -p deep start && cp .migrate snapshots/${time}.migrate`);
