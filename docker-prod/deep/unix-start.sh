...
COMPOSE="$(docker-compose version --short)" MAJOR=${COMPOSE:0:1}; if [ "$MAJOR" = "2" ];  then export COMPATIBILITY='--compatibility'; fi; docker-compose -p deep $COMPATIBILITY down -v --remove-orphans && docker-compose pull && docker-compose -p deep $COMPATIBILITY up -d
sleep 5
echo 'Migrating data'
sleep 5
